...
class EditKategoriScreen extends StatefulWidget {
  final String id;
  final String namaKategoriLama;

  const EditKategoriScreen({
    super.key, 
    required this.id, 
    required this.namaKategoriLama,
  });
...
class _EditKategoriScreenState extends State<EditKategoriScreen> {
  ...
  void initState() {
    super.initState();
    _namaController = TextEditingController(text: widget.namaKategoriLama);
  }
  ...
  void _simpan() async {
    ...
    bool sukses = await _apiKategori.putKategori(widget.id, _namaController.text);
    ...
    if (sukses) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Gagal mengedit kategori')),
        );
      }
    }
  }
